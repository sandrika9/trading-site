...
app.config['MAIL_USERNAME'] = os.environ.get("MAIL_USERNAME", "your_email@gmail.com") # შეგიძლია ცვლადებში შეიტანო ან პირველ ეტაპზე აქ ჩაწერო
...
def register():
    if request.method == "POST":
        username = request.form.get("username")
        email = request.form.get("email") # ახალი: მეილის მიღება ფორმიდან
        password = request.form.get("password")
        ...
        try:
            ...
            # ვამოწმებთ ხომ არ არსებობს უკვე ასეთი იუზერი ან მეილი
...
def forgot_password():
    if request.method == "POST":
        ...
        if user:
            ...
            expiration = datetime.utcnow() + timedelta(hours=1) # ტოკენი ძალაშია 1 საათი
